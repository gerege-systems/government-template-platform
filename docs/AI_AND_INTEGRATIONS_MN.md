...
Энэ бол **үндсэн буух хуудасны нэвтрэлт** — **DAN** брэндтэй (dgov-ийн үндэсний SSO,
хэрэглэгчийн портал dan.dgov.mn), иргэн өөрийн eID аппаар баталгаажин нэвтэрдэг.
**Ory Hydra** IdP-д зориулсан хамгийн бага Authorization-Code клиент; `SSO_ISSUER`-ийн
default нь `https://sso.dgov.mn` (deployment дээр DAN-ий issuer рүү тохируулна).
Endpoint-ууд нь issuer-ээс гаргагдана (`/oauth2/auth`, `/oauth2/token`, `/userinfo`,
...
- **DAN SSO нэвтрэлт**: `ASWebAuthenticationSession`-аар native OIDC + PKCE (public client `template-dgov-mn-ios`) → `POST /api/auth/sso/native`.
