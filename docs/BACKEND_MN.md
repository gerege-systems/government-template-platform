...
- **auth** — хамгийн том; eID нэвтрэлтийн амьдралын мөчлөг (`EIDStart`, `EIDStartByNationalID`, `EIDPoll`), session (`Refresh`, `Logout`), Google (`GoogleLogin`, холболт салгах), eID org холболт + signers, eID профайл унших. eID бол үндсэн нэвтрэлтийн зам.
